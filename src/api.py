...
# Redis databases
rd = redis.Redis(host=os.environ.get('REDIS_HOST', 'redis-prod'), port=6379, db=0)  # Raw UFO data
rdb = redis.Redis(host=os.environ.get('REDIS_HOST', 'redis-prod'), port=6379, db=3)  # Job results
...
def create_job():
    try:
        ...
        logger.info(f"Creating job with start: {start_date}, end: {end_date}")
        job = add_job(start_date, end_date)
        logger.info(f"Job successfully submitted: {job['id']}")
        return jsonify(job)
